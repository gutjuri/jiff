...
function JIFFClient(hostname, computation_id, options) {
  ...
  options.maxInitializationRetries =
    options.maxInitializationRetries || constants.maxInitializationRetries;
  if (typeof options.Zp === "number" && options.safemod !== false) {
    // big numbers are checked by extension
    if (!this.helpers.is_prime(options.Zp)) {
      throw new Error(
        "Zp = " +
          options.Zp +
          " is not prime.  Please use a prime number for the modulus or set safemod to false."
      );
    }
  }
   ...
   */
  this.sodium_ =
    options.sodium !== false ? require("libsodium-wrappers") : false;
  ...
  this.socketConnect = function (JIFFClientInstance) {
    console.log("called jiff-client.socketConnect");
    if (options.__internal_socket == null) {
      ...
      console.log("made guarded socket");
    } else {
      console.log("no internal socket");
      JIFFClientInstance.socket = internalSocket(
        JIFFClientInstance,
    ...
    console.log("finished jiff-client.socketConnect");
  };
  ...
  this.connect = function () {
    ...
    console.log("jiff-client.connect");
    ...
    console.log("finished jiff-client.connect");
  };
...
console.log("finished jiff-client: JiffClient");
// Add socket event handlers to prototype
socketEvents(JIFFClient);
console.log("finished jiff-client: socketEvents");
// Add extension management to prototype
extensions(JIFFClient);
console.log("finished jiff-client: extensions");

console.log("finished jiff-client");
