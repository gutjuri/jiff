...
module.exports = function (JIFFServer) {
  ...
  JIFFServer.prototype.compute = function (computation_id, options) {
    ...
    this.computation_instances_map[computation_id] =
      create_computation_instance(this, computation_id, options);
...
function create_computation_instance(jiff, computation_id, options) {
  ...
  // Create instance
  var computation_instance = new client(
    "<server_instance>",
    computation_id,
    options
  );
...
InternalSocket.prototype.connect = function () {
  var computation_instance =
    this.jiff.computation_instances_map[this.computation_id];
  ...
  var output = this.jiff.handlers.initializeParty(
    this.computation_id,
    "s1",
    computation_instance.party_count,
    msg,
    true
  );
  ...
  if (output.success) {
    computation_instance.secret_key =
      this.jiff.computationMaps.secretKeys[this.computation_id];
    computation_instance.public_key =
      this.jiff.computationMaps.keys[this.computation_id]["s1"];

    computation_instance.socket.receive(
      "initialization",
    ...
    throw new Error(
      "Cannot initialize computation instance " +
        this.computation_id +
        ". Error: " +
        output.error
    );
